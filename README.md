...
**Status:** v0.12.2. Dashboard, envelope budgets, bulk categorization, transactions list, subscriptions, goals and the 6-month trend chart all ship. `/sync` pulls posted transactions straight from the bank, and its balance check now tells real drift from a stale bank figure instead of flagging both the same way; `/import` still handles anything the feed's 45-day window no longer reaches, and can now fix a wrong starting-balance anchor inline — the anchor move itself is now atomic with the import, bounds-checked, and reversible from the import success page. Auto-categorization at import time (CSV or sync) can now be undone per-batch, reverting just the categorization without discarding the rest of the import. The app also runs in Docker now (still on SQLite — see the [Docker](#docker) section below). See [PLAN.md](./PLAN.md) and [CHANGELOG.md](./CHANGELOG.md).
