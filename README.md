...
A credit card carries a credit limit and a minimum payment, both editable after the fact, and its row shows utilization against that limit, when the balance was last confirmed, and how much you paid it down this month. Four things move a liability's balance, and a row only ever offers the one that can work for it:

- **Reconcile** — type what the card actually owes today, as a positive number. This is the normal way a card's balance moves, and it is the only way for a card that isn't linked to the feed.
- **Refresh** — pull the balance straight from the bank feed. Offered only where it can work: a feed-linked account with no transactions of its own, which in practice means the mortgage.
...
Every balance change records the balance it replaced, and the row offers to go back to it. That undo is itself undoable, so it's a toggle rather than a one-shot. Only one prior balance is kept, though — there's no history series behind it.

A mortgage or a loan is deliberately quieter: no utilization bar, no Reconcile, no hand-entered charges, no paid-down line. It sits under a **Long-term** heading, counts toward net worth, and otherwise stays out of the way — it's a fact about your life, not a problem you're solving this month.
