...
| Post-commit refresh warning ("saved, but this page couldn't refresh") | `components/ledger/action-status.tsx`'s `StatusWarning`, shared by `/accounts`, `/import` and `/goals` | Yes — 18% / 45%, the same formula `sync/ActionForm.tsx`'s warnings list uses for the byte-identical sentence |
...
**Added v0.27.0:** two more, both on the shared formula. The refresh warning is the one that matters for blast radius — it is now rendered by THREE routes (`/accounts`, `/import`, `/goals`) through one component, across five call sites, which is the first time a single amber surface has had more than one caller. The charge dialog's "switches it to Reconcile" panel is the other, and it is the token's first use as a *consequence-of-this-action* notice rather than a state.
