...
**Added v0.27.0:** two more, both on the shared formula. The refresh warning is the one that matters for blast radius — it is now rendered by FOUR routes through one component, which is the first time a single amber surface has had more than one caller. The charge dialog's "switches it to Reconcile" panel is the other, and it is the token's first use as a *consequence-of-this-action* notice rather than a state.
