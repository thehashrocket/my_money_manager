...
**Added by the card-transaction-import plan (2026-09-09):** a third consequence-of-this-action notice, on the same 18%/45% formula — `ReconcileForm`'s (`_balance-forms.tsx`) warning that reconciling an importing card's balance permanently excludes any not-yet-synced charge (D8.1's cutover, same account, same page as the charge dialog above). Found and fixed by the design specialist during that plan's `/ship` review: the first draft used plain `text-xs text-muted-foreground`, which under-weighted a comparable-stakes warning sitting on the same form.
