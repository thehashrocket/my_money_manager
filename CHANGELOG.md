...
- **The "anchor moved during the fetch" warning above could overstate how many transactions a race actually cost, and could fire on the wrong kind of anchor move entirely.** It counted every transaction a card's fresh anchor excluded once the anchor had changed at all — including ordinary pre-anchor history a newly-linked card's lookback routinely contains regardless of any race — and didn't check which direction the anchor moved, so undoing a sync (which can only make more transactions eligible, never fewer) could still trigger "use Undo... then sync again" for transactions nothing actually stranded. The warning now counts only the specific transactions the race cost.
- **A second sync landing the same transactions while this run was mid-fetch could, in one specific case, report "up to date" with no explanation at all** — instead of the "already imported by another sync" warning every other flavor of this race already surfaces — and could report a stale transaction count for the affected account. Closed by bringing that check up to the same standard as the two fixes above.
...
### Fixed
- **Running the app in Docker could silently lose your database backups over time.** Every container restart took a fresh snapshot and trimmed old ones down to the last 10 — even an ordinary restart with no schema change at all. Ten restarts after any real upgrade, that upgrade's own backup could get pushed out by backups of nothing happening. Snapshots and cleanup now only run when a database upgrade is actually about to happen.
- **A database backup that failed partway (for example, a permissions problem on the backups folder) used to just print a warning and boot anyway — including right before an upgrade with no working backup to fall back on.** Since backups are now only taken right before an upgrade, a failed one means exactly what it says: there's no safety net for the change about to happen, so the app now refuses to start and tells you how to fix it (and, if the failed backup left a partial file behind, keeps it out of the way of your good ones rather than letting future cleanup mistake it for something worth keeping).
...
### Fixed
- **Reconciling a credit card to a positive balance (money owed to you, after an overpayment) silently stored it as a negative balance instead.** The Reconcile form had no way to say which direction a typed amount went — it always assumed you owed money. A "You owe" / "You're owed" toggle now sits next to the amount, defaulting from the account's current balance. A loan or mortgage never offers this toggle at all, since it can never legitimately carry a positive balance.
- **That toggle could silently flip back to "You owe" on a second save, discarding your pick.** Browser form resets after a save reverted the toggle's on-screen state even though nothing was touched. Fixed by using two buttons instead of native radio inputs, the same pattern already used for the amount and date fields on this form.
- **Re-clicking the already-selected toggle option could freeze the amount field on a stale figure.** If you clicked "You owe" (already selected) and then added a charge to the same card from elsewhere on the page without closing the Reconcile form, the amount shown no longer updated to include it — so saving could understate what you owe. The field now only stops auto-updating when you actually change something.
- **A screen reader announced the amount field backwards.** It described "You owe" as money owed *by* the account and "You're owed" as money owed *to* the account — the reverse of what the visible toggle says.
