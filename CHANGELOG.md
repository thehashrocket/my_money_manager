...
- **The warning explaining why Remember did nothing is no longer hidden behind the success message.** Stacked notifications collapse so that only the newest is fully readable, and the warning was being sent first — so on `/transactions`, where the checkbox cannot grey itself out ahead of time and the toast is the only explanation you get, it was the one message you could not read. It now arrives last.
- **The greyed-out Remember checkbox's explanation is now attached to the checkbox**, not just printed near it, so a screen reader announces the reason instead of an unexplained disabled control. On narrow screens that explanation also stops right-aligning itself under a left-aligned row, and it is no longer set smaller than the label it explains.
