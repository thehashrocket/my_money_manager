...
- **A refusal you are supposed to read is no longer wiped off the screen before you can read it.** Three cases where `/sync` declines to do something — undoing a sync that has already been undone, undoing one that is no longer safe to undo, unlinking a pair that is already unlinked — refreshed the page as part of declining, which could remove the very message explaining why. None of them had changed anything, so there was nothing to refresh for. Recording a rejection had the mirror-image problem: it refreshed inside the step that catches failures, so a hiccup during the refresh would report an already-saved decision as having failed.
