...
- **Not yet verified on real Linux/NAS hosts:** `./backups` is a bind mount, and Docker on native Linux auto-creates a missing bind-mount host directory as root-owned. The Dockerfile's `chown` only affects the image filesystem, which the bind mount then shadows — so a first run on a Linux host could fail every snapshot write with EACCES (dev/testing here has all been on macOS Docker Desktop, whose bind-mount layer is more permissive). `mkdir -p ./backups && sudo chown 1000:1000 ./backups` on the host before the first `docker compose up` is the workaround until this is addressed properly, which belongs with PR3 (NAS deployment).
