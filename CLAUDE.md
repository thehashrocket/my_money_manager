...
1. **All money is stored as signed integer `amount_cents`.** Never floats. Never a separate sign column. Withdrawals are negative. Deposits are positive. Balance = `starting_balance_cents + SUM(amount_cents WHERE date > starting_balance_date)`.
   ...
   A wrong anchor is no longer a raw-SQL fix, either: `updateAccountAnchorAction` (`src/app/import/actions.ts`) plus `validateUpdateAnchorInput` (`src/lib/import/validateUpdateAnchorInput.ts`) give `/import` an inline per-account form to set `(starting_balance_cents, starting_balance_date)` directly — the only way back once `deriveStartingBalance`'s forward-only anchor has already moved too late. The date is capped at today: a future anchor would exclude every real transaction from the sum and permanently silence the freshness check above for that account too.
