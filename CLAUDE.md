...
- **On real Linux hosts (confirmed via CI, not just theorized), `./backups` needs its ownership fixed before the first `docker compose up`.** It's a bind mount, and Docker on native Linux auto-creates a missing bind-mount host directory as root-owned; the Dockerfile's `chown` only affects the image filesystem, which the bind mount then shadows. Without this, the container's unprivileged `node` user gets `EACCES` on its first snapshot write and never becomes healthy — this actually broke CI (which runs on Ubuntu) the first time the `docker` job ran, whereas dev/testing on macOS Docker Desktop never surfaced it (its bind-mount layer is more permissive). Fix: `mkdir -p ./backups && sudo chown 1000:1000 ./backups` on the host before the first `docker compose up` — CI's `docker` job does this as its own step.
