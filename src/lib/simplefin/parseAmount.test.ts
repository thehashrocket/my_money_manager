...
import { parseAmountToCents, SimpleFinAmountError } from "./parseAmount";
...
describe("parseAmountToCents", () => {
  ...
  it("rejects anything it cannot parse exactly", () => {
    for (const bad of ["", "abc", "1.2.3", "1,000.00", "$5.00", "--1", "1e3"]) {
      expect(() => parseAmountToCents(bad)).toThrow(SimpleFinAmountError);
    }
    // @ts-expect-error guarding the runtime boundary, not the type
    expect(() => parseAmountToCents(null)).toThrow(SimpleFinAmountError);
  });
});
