...
- [ ] **T7 (P1, human: ~5h / CC: ~45min)** — read model — income sections, bands from `kind`, `leftToBudgetCents`, `fundCount`, month-scopable backlog; **`Uncategorized` returned as its own field, conditional on spend-or-backlog (DS26)**; **two-level `sort_order` ordering (DS29/DS12)**; **band subtotal figures exposed for DS27**
