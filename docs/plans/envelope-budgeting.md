...
- [x] **T10 (P2, human: ~45min / CC: ~12min)** — docs — **five** stale JSDoc blocks (C2 + E9); B3's rationale comment **and O5's beside it (E4)**; `invalidateForwardRollover`'s contract rewritten to say the read branch is unreachable (P3); inline ASCII diagrams in `budget.ts` (two-sign dispatch), `copyMonth.ts` (pipeline), `resolveRowDisplay.ts` (rule table), **`loadMonthView.ts` (E9 — the band/group/leaf axes, the hardest structure in this plan to reconstruct from the code)**
