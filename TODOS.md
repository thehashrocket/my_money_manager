...
- [ ] **P2** — **PR3: what a fund's progress MEANS, and the paydown envelope that waits on the answer.** (Merged 2026-09-09: this was filed three times — as part (b) of "PR3: fund behavior unification" here, as the `categories.account_id` P2 in the liability review, and again as a P3 in the 2026-09-08 triage. Part (c) of the original entry, the overspent-fund carry question, moved to **"The three rollover open questions"** below, where its two siblings already live. Part (a) — funds rendering as budget rows rather than living only on `/goals` — closed 2026-09-08 in v0.23.0 when the FUNDS band became editable and started writing `budget_periods` through the same `upsertAllocation` the other two bands use.)
...
- [ ] **P2** — **The fourth carry-forward gate is asserted against the serializer, not against the form.** (Merged 2026-09-09: filed twice across consecutive `/ship` passes; this is the sharper statement of the same hole.) `_filter-bar.test.ts` proves `VISIBLE_FIELDS` and the serializer partition cleanly, which catches a typo'd field name. It does not catch the case its own comment names: rename `<select name="pending">` to `name="status"` and leave `"pending"` in `VISIBLE_FIELDS`, and every assertion passes while "Apply filters" silently drops the Status filter on every submit — the silent-page-2 class the whole contract exists to prevent. Typing `VISIBLE_FIELDS`' elements as `keyof TransactionsFilterValues` catches a typo but NOT a rename that moves both sides. `vitest.config.mts` is `environment: "node"`, so nothing in the suite can see rendered markup; closing it properly needs either a jsdom project (or a `renderToStaticMarkup` pass asserting every `VISIBLE_FIELDS` name appears as a `name=` on a non-hidden control) or a source-text scan asserting `VISIBLE_FIELDS` equals the set of `name="..."` literals in the file. That is test infrastructure this repo does not have yet, and CLAUDE.md rules UI-component tests out of V1. Blocked by: a decision on whether a form-contract test counts as the UI-component testing CLAUDE.md rules out of V1. (`src/app/transactions/_filter-bar.test.ts`, `vitest.config.mts`)
...
- [ ] **P2 — the per-component decision logic (`pendingCategoryId`, `trainability`, `ruleAction`, `ruleActionEnabled`, the `handlePick` consent-reset lookahead, the `<p id={reasonId}>` reason block) is hand-duplicated across `_merchant-row.tsx` and `_transaction-row.tsx` — roughly 40 lines, confirmed near-byte-identical by BOTH the code-reviewer and type-design-analyzer passes independently.** This is the exact defect class CLAUDE.md names repeatedly elsewhere in this codebase (`action-status.tsx`, `resolveCardAffordances.ts`, `kindsImplyUsed.ts`) — and it already bit THIS branch once: round 2 found `MerchantRow` missing the revalidation-driven consent reset `TransactionRowForm` had, precisely because the same logic lived in two places and only one got updated. It matters more here than usual because CLAUDE.md rules UI-component tests out of V1 — logic living in `.tsx` cannot be tested at all, while the identical logic folded into `keyTrainability.ts` (zero runtime imports) already is. Two independent shapes were proposed and neither was applied in this pass: (a) fold `classifyKeyTrainability` + `describeRuleAction` into one pure `resolveRememberUi(key, filedCategoryIds, existingRule, pendingCategoryId) → {action, enabled, message, label}`, removing the possibility (currently untyped) of passing `describeRuleAction` a `verdict` computed from a DIFFERENT key than its own `normalizedMerchant` argument; (b) replace the four independent `remember`-clearing call sites (two `handlePick`s, two `prevRow`/`prevGroup` prop-reset blocks) with one derived value — store the `RuleAction["kind"]` consent was given for and mask on any mismatch (`checked = consentedKind === ruleAction.kind`), which is automatically correct for every future path that can move the verdict, not just the two known today. Both are real refactors with their own risk (touching working, already-twice-adversarially-reviewed code), not a follow-up-comment-sized fix — deliberately not attempted in the same pass that already made five other changes to the same files. Blocked by: a calmer, dedicated session; the individual duplication this PR could safely extract without restructuring render logic (the message ternary, the label ternary) already was. (`src/app/categorize/_merchant-row.tsx`, `src/app/transactions/_transaction-row.tsx`, `src/lib/categorize/keyTrainability.ts`)
