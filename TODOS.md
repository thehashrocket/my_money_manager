...
several DRY/documentation findings (also fixed) and one accepted residual. A
FOURTH review round (2026-09-18, an independent PR review after the branch was
already open) found two more real bugs the first three rounds missed — the
cutover-recheck warning's "3/10, in principle" residual above was neither
narrow nor in-principle, and the id-race recheck had never been brought up to
the standard the two new rechecks set — both fixed, see their own entries
below:

- [x] **P1 → FIXED same session (red-team finding, confidence 9/10, empirically reproduced).** **Widening the staging loop to fix the anchor-backward race broke `recheckCutoverAnchor`'s "routine drop = no warning" invariant** — every ordinary D8.1 cutover exclusion (not just a genuine anchor-moved-during-fetch race) started firing the "landed on or before its balance date... use Undo on /accounts... then sync again" warning, directly contradicting this file's own documented intent a few hundred lines below ("the drop is routine, so announcing it every run would be the noise D4.3 exists to remove"). Reproduced live: a newly-linked card's feed lookback routinely spans weeks of pre-anchor history before its first post-cutover activity (R1's own comment), so this would have fired the scary, actionable-sounding false alarm on every single ordinary sync in that window. **Fix:** `recheckCutoverAnchor` now compares the freshly-read anchor against `entry.account.startingBalanceDate` — the value staging itself observed, carried on the same account object — and only warns when they differ; a row is still dropped and counted either way, only the WARNING is gated on a genuine race. Two regression tests added directly to existing no-race tests (asserting `outcome.warnings` carries nothing matching "balance date"), both confirmed to fail against the pre-fix code and pass with it. A second, independent adversarial pass verified the fix has no false-negative case (if the anchor is unchanged, the dropped row set is provably identical to what the old pre-filter would have produced) and found one negligible (3/10) residual: the gate is per-ACCOUNT, not per-row-cause, so a genuinely raced account's warning could in principle fire for a row whose own eligibility didn't actually depend on the moved anchor — narrower than the bug this fixes, not acted on.
      **Correction, follow-up review (2026-09-18): "3/10, in principle" was WRONG — the case is neither narrow nor merely in-principle, and is now fixed rather than accepted.** Reproduced directly: a card whose anchor moved forward mid-fetch past 3 routine pre-anchor rows and 1 genuinely raced row reported "4 transactions... weren't imported" for a race that cost exactly 1 — R1's own comment (a newly-linked card's lookback routinely spans WEEKS of pre-anchor history) means this overstatement is the ORDINARY shape of the warning, not a corner case. Worse, the gate never checked DIRECTION: `isAfterAnchor` is `date > anchor`, so a BACKWARD anchor move (the user hitting Undo in another tab) can only ever add eligible rows, never remove them — anything still dropped after an earlier anchor would have been dropped by a later one too, meaning nothing was actually raced — yet the old gate fired anyway, telling a user who had just hit Undo to hit Undo again, which SWAPS the account's single `prior_starting_balance_*` slot (rule 9) on a false alarm for rows nothing stranded. **Fixed**: the warned count is now the SET of rows eligible under the OLD (staging-time) anchor but not the fresh one, computed per row rather than gated per account — a formula that naturally excludes routine drops sharing an account with a real race AND naturally yields zero for a backward move, with no separate direction check needed. Two regression tests confirmed to fail against the pre-fix code and pass with it (`sync.test.ts`, "warns for only the RACED subset of a forward anchor move..." and "does NOT warn when a backward anchor move..."). (`src/lib/simplefin/sync.ts`, `src/lib/simplefin/sync.test.ts`)
- [ ] **P4 — accepted residual, not fixed: the `totalToInsert === 0` early-return no longer takes the cheap path for a routine pre-cutover-only card sync (red-team finding, confidence 8/10).** Because pre-cutover rows now always enter `toInsert` at staging (the fix above removes the early skip), a card whose feed sends only routine, pre-anchor history with nothing new past the anchor no longer takes the cheap "up-to-date" early return — it now unconditionally takes the full write path (`createSnapshot`'s real `VACUUM INTO`, opens a `db.transaction`, inserts nothing, hits `verifiedTotal === 0`, throws `NothingVerifiedError`, deletes the snapshot it just took) on every such sync. `NothingVerifiedError`'s existing handling already deletes the snapshot rather than let it evict a real one from the retention pool, and no batch is minted — the remaining cost is unnecessary disk I/O and an open-then-rollback transaction, on a manual "Sync now" button (not a cron), for one card, during the "weeks" window R1's own comment names before a newly-linked card's first post-cutover activity. Fix direction, if this is ever worth doing: have the early-return check "would any row plausibly survive the cutover" for card accounts rather than raw `toInsert.length`, without reopening the same read-before-await staleness rule 11 exists to close. Blocked by: nothing urgent — deferred rather than fixed under `/ship` review pressure for a non-correctness, low-frequency cost.
      **Correction, same session: "no correctness impact" (this entry's original claim) was WRONG — fixed instead of left as stated.** A Codex structured-review pass on this exact residual found that reaching `NothingVerifiedError` more often ALSO meant more often reaching a blanket `counts.map()` that unconditionally blanked `reportedBalanceCents`/`availableBalanceCents`/`balanceDate`/`duplicateByExternalId`/`duplicateByContent` for EVERY staged account — not just the ones this rollback path actually concerns. That zeroing predates this branch and was correct under the assumption `NothingVerifiedError` could only mean "every account was link-dropped" (same class of now-false assumption as the P1 fix above, a third instance of it in this same function). Once a routine, unrelated zero-net-row account can ALSO reach this path, its genuine feed balance and duplicate counts were being thrown away too — silencing that account's own rule-1 drift check for the run. Empirically confirmed (an unrelated checking account with a real $1,000 reported balance came back `reportedBalanceCents: null`) and fixed: the blanking now applies only to accounts in `dropped` (genuinely link-dropped); a surviving account's already-correct `insertedCount` (0, either from having nothing staged or from `applyDedupPruning` already bringing it there) and its real balance/duplicate fields both pass through untouched. Regression test confirmed to fail against the pre-fix code and pass with it (`sync.test.ts`, "preserves an UNRELATED account's real reported balance and duplicate counts..."). The *unnecessary snapshot I/O* half of this entry's original claim still stands and is still deferred. (`src/lib/simplefin/sync.ts`, `src/lib/simplefin/sync.test.ts`)
...
- [x] **P1 → FIXED, follow-up review (2026-09-18, independent PR review).** **The pre-existing id-race recheck was never brought up to the same standard as the two NEW rechecks this branch added — it had no rollback-path rebuild, even though `NothingVerifiedError`'s own comment already named "raced-id-only" as one of the ways every staged account ends up dropped.** Concretely: two overlapping `/sync` runs landing the same external ids for the only staged account throws `NothingVerifiedError` (nothing survived), and the rollback `catch` block explicitly rebuilt the content-recheck and cutover-recheck warnings/counts against the live handle — but had no equivalent for the id pass. The write path's own id-race warning ("N transactions had already been imported by another sync...") and its `duplicateByExternalId` adjustment both lived exclusively inside the transaction the rollback discards, so a purely id-raced rollback reported `up-to-date` with NO warning at all and stale, staging-time `insertedCount`/`duplicateByExternalId` — the exact fabricated-fact class rule 1/5/11 exist to prevent, reached through the one recheck the three prior review rounds on this branch never extended. A second, smaller gap in the same code: the id-race warning was the only one of the three rechecks that didn't name the account (an aggregate "N transactions had already been imported..." sentence, where its two new siblings are both per-account). **Fixed**: the inline id-race block was extracted into `recheckLandedIds`, the same `AnyDb`-dual-use shape as `recheckContentDedup`/`recheckCutoverAnchor`, made per-account (closing the naming gap too), routed through `applyDedupPruning`'s existing bookkeeping tail (`countField` widened to accept `"duplicateByExternalId"`), and wired into BOTH the write path (replacing the inline block) and the `NothingVerifiedError` rollback path (as the FIRST recheck in the chain, matching the write path's own id → content → cutover order). Two regression tests confirmed to fail against the pre-fix code and pass with it (`sync.test.ts`, "warns (naming the account) and reports accurate counts when a concurrent sync lands the ONLY staged row's external id..." and "adjusts per-account counts for a PARTIAL id-race drop on the ordinary SUCCESS path too" — the latter closing a related pre-existing gap: even on the ordinary success path, a partial id-race drop never adjusted the per-account `counts`, only the aggregate `insertedCount` was correct). Also fixed in the same pass, unrelated bug found by the same review: `recheckContentDedup`'s lookback floor (`contentFloorIso`) was independently re-derived from `now` at both staging time and recheck time, agreeing only because both call sites happened to pass the same `now` — nothing enforced it, and a future caller passing a different `now` (or scoping the recheck differently) would silently reopen the race this whole branch exists to close, or manufacture false drops, with no test able to catch it. Now computed ONCE at staging time and carried on the `Staged` entry, so the two agree by construction. (`src/lib/simplefin/sync.ts`, `src/lib/simplefin/sync.test.ts`)

- [x] **P2 → FIXED (2026-09-18, `sync-cutover-reissue-fixes` branch, PR2 of `docs/plans/sync-pending-promotion.md`).** This IS introduced by this branch's own widening, unlike the two entries above. Before the widening, a pre-cutover occurrence was filtered out of the staging loop BEFORE it ever reached the `seenExternalIds` within-response-duplicate check, so it could never "claim" the id ahead of a later, eligible occurrence. Now every non-pending row (regardless of cutover eligibility) flows through id-dedup uniformly, so if SimpleFIN sends the SAME external id twice in one response — a real, if rare, anomaly this codebase already has tests for elsewhere ("stays QUIET when the SAME external id appears TWICE in one feed response... a real feed sending a repeated transaction id is not something anything validates against upstream of this loop") — and one occurrence is dated on-or-before the anchor while the other is genuinely after it, the pre-anchor occurrence (processed first, in array order) wins the id slot via `seenExternalIds.add()`, and the post-anchor, eligible occurrence is dropped as `duplicateByExternalId` before it ever reaches the cutover recheck. Both occurrences are then lost: the pre-anchor one correctly (cutover), the post-anchor one incorrectly. Reproduced empirically with a scratch test (anchor `2026-08-20`, one occurrence dated `2026-08-15`, one dated `2026-09-01`, same external id): `ROWS: []` — neither lands, confirmed against BOTH occurrences of the same id producing zero inserted rows where the post-anchor one should have survived. **Fixed**: the staging loop no longer picks a single within-response-duplicate "winner" by array order at all. Every not-known-before-this-run row is grouped by external_id first; a group with 2+ genuinely differing signatures is pushed through UNCOLLAPSED (each occurrence carrying its own real date), and `recheckCutoverAnchor` — which already runs inside the write transaction, against the FRESH anchor — is what then drops the ineligible occurrence(s) and keeps the eligible one, per row, using each row's own real date rather than any tie-break heuristic. A same-session adversarial review (Codex adversarial + Codex structured review, `/ship` Step 11) found two further interaction bugs in this exact design before it shipped — a resource match on one occurrence not blocking a differing-signature sibling from also inserting, and a promotion reservation being strandable when its bearer occurrence lost the write-time identity race — both closed by resolving each external_id group as a UNIT (any resource match found on any occurrence speaks for the whole group) rather than deciding occurrence-by-occurrence; see the commit fixing those findings for the full reasoning. (`src/lib/simplefin/sync.ts`, `src/lib/simplefin/sync.test.ts`)

- [x] **P2 → FIXED (2026-09-18, `sync-pending-promotion` branch, `/plan-eng-review` + `/ship`).** **A posted feed row racing against a genuinely PENDING existing row with a matching content signature is silently dropped by content dedup — permanently, since the pending row is never promoted to posted and the transaction stays excluded from the balance sum forever (rule 1's `SUM(... AND NOT is_pending)`).** Reproduced with NO race involved at all (pure, ordinary staging-time flow — `createSnapshotMock` untouched): a pending CSV row and a matching posted feed row, same date/amount/memo, produce `duplicateByContent: 1, insertedCount: 0`, the pending row stays `isPending: true` forever, and `driftCents: 487` (a phantom missing $4.87) is reported on every future sync. `loadContentBudget`'s candidacy query (`existingByContent`, unchanged by this branch beyond the byte-identical extraction into a shared function) has no `is_pending` filter, so a pending existing row competes for a content-dedup budget slot exactly like a posted one — but unlike CSV import's `commitImport`, which has dedicated logic to UPDATE a matched pending row in place when its posted counterpart arrives (rule 3: "A content match against an existing pending row is not just dropped... commitImport updates that row in place"), `syncSimpleFin`'s own content dedup has no equivalent promotion path — it just drops the posted row as a duplicate and leaves the pending one exactly as it was. Genuinely out of scope for this branch: this is a pre-existing gap in the ORIGINAL staging-time content-dedup design (present on `main` before this branch's changes), not something the race-condition fixes here introduced or worsened — my new `recheckContentDedup` never even runs differently for this case; the row is already gone by staging time, before the new recheck ever sees it. How often a bank-fed pending CSV row and a later-posted SimpleFIN row for the SAME transaction would actually collide in practice is unmeasured — Star One via MX is documented (CLAUDE.md) as posting-only over the feed, so this specific collision needs a CSV-imported pending row (itself an edge case — CSV import IS the pending-eligible path) to still be sitting unresolved when the SAME transaction later arrives over the feed. Recorded here rather than silently discovered later; not sized or fixed as part of this branch. (`src/lib/simplefin/sync.ts`)

## Follow-ups from the `sync-pending-promotion` branch (2026-09-18, `/plan-eng-review` + `/ship`)

Fixes both TODOS.md entries above via a `/plan-eng-review` triage of TODOS.md/PLAN.md: the pending/posted content-dedup promotion gap (now closed, see the entry above) and, sketched but not yet built, the D8.1 cutover-boundary duplicate-id loss. Three outside-review rounds (Codex) ran against the plan before implementation began, each finding real structural gaps in the previous round's design — see `docs/plans/sync-pending-promotion.md` for the full history (batch/undo model, recheck-bypass, transfer-pairing breakage, then transfer-pair-undo asymmetry and `pairsLinkedCount`, then prior-pairing-state restore and `bankTransactionNumber` nulling). New `sync_promotions` table records a promoted row's pre-promotion snapshot so `undoSyncBatch` can revert it exactly (including restoring any transfer pairing that predates the promotion) instead of only ever deleting rows.

- [x] **P2 → FIXED for the RACE case (2026-09-18, `sync-cutover-reissue-fixes` branch, PR2 of `docs/plans/sync-pending-promotion.md`) — the permanent-reissue case remains an accepted residual, see the P4 entry below.** **A posted row tagged with THIS feed's own provenance under a DIFFERENT external id than an incoming row for what is really the same real-world transaction is excluded from content-dedup candidacy entirely, and — since the ids differ — also missed by the id pass, producing a genuine duplicate row.** `loadContentBudget`'s candidacy query (`queryContentDedupCandidateRows`, unchanged by this branch beyond the byte-identical extraction and its own `isPending` tally-filter) excludes a row already carrying `simplefin_source_account_id = feedId` from content-dedup candidacy on the documented assumption that "the id pass already accounts for it" (case 1-3 in its own comment) — true when the SAME feed issues a STABLE id for an already-posted transaction across repeated fetches, which is the normal case, but false if the same feed genuinely issues two different ids for what is really one event across two overlapping fetches. Reproduced empirically: seed a pending row, then in a mocked `fetchAccounts` side effect simulate a concurrent sync stamping it posted under external id `"ext-concurrent-1"`, while the CURRENT fetch's own response returns the same content under a DIFFERENT id `"ext-race2-1"` — result is two rows for one real transaction, confirmed via `rows.toHaveLength(2)` before the test was corrected to the realistic same-id scenario. Verified pre-existing (not introduced by this branch) by diffing `queryContentDedupCandidateRows`'s WHERE clause byte-for-byte against `main`'s original `existingByContent` query — identical. **Narrowness, for calibration:** requires SimpleFIN to issue two DIFFERENT external ids for what is really the same real-world transaction across two overlapping fetches of the SAME account — a much stronger assumption violation than the already-accepted "same feed, same id, sent twice in one response" case tested elsewhere in this file; no evidence this has ever happened on the live ledger. **Fixed, for the race shape**: `recheckReissuedIds` (`src/lib/simplefin/sync.ts`) takes a pre-fetch snapshot of every same-feed row already on the account (`querySameFeedRowsSince`, captured BEFORE `await fetchAccounts`) and, inside the write transaction, diffs a fresh read of the same population against it — a row that appeared strictly during THIS sync's own fetch window is treated as reissue evidence and multiset-matched (via `consumeBySignature`, shared with `recheckContentDedup`) against the incoming batch, dropping only a genuine race loss rather than every routine, steady-state exclusion `queryContentDedupCandidateRows` already handles. Deliberately does NOT touch `queryContentDedupCandidateRows`'s candidacy exclusion itself — see the P4 entry below for why the PERMANENT-reissue case (no race, no concurrent sync, the feed durably switches ids) remains open. (`src/lib/simplefin/sync.ts`)
- [ ] **P4 — accepted residual, disclosed rather than fixed (found while designing PR2's fix for the entry above, `/plan-eng-review` 2026-09-18).** PR2's `recheckReissuedIds` (`docs/plans/sync-pending-promotion.md`) only closes the RACE shape of the entry immediately above — two overlapping fetches — because it's scoped to same-feed rows that appeared strictly during THIS sync's own fetch window. If a reissued id is ever PERMANENT rather than a one-off race artifact (the feed genuinely, durably switches to reporting a new id for an already-imported transaction, not just during a concurrent-sync collision), every later ORDINARY sync reporting that new id will still insert a duplicate row — the fix deliberately never touches `queryContentDedupCandidateRows`'s steady-state candidacy exclusion of same-feed rows, because doing so reopens the coincidental-same-day-content regression rule 3 already fixed once (two genuinely identical same-day transactions must both survive; naively relaxing candidacy would make an old, unrelated same-feed row wrongly consume a new, legitimately-different transaction's content-budget slot). Not fixed because it needs its own design pass distinguishing "this old row's id was durably replaced" from "this old row coincidentally shares content with a new, different transaction" — the same content-alone ambiguity rule 4 documents for same-account reversals — and there is zero evidence a permanent (non-race) reissue has ever happened. If this ever needs solving, start from PR2's own round-1/round-2 outside-review history in `docs/plans/sync-pending-promotion.md`, which already worked out why the obvious-looking fix (just widen candidacy) is wrong. (`src/lib/simplefin/sync.ts`)

## Follow-ups from the `/ship` pre-landing + adversarial review (2026-09-18, sync-pending-promotion)

Three specialist+red-team review rounds ran against this branch (6 specialists, then two further red-team convergence passes). Found and fixed 10 issues, 4 of them CRITICAL: `revertPromotion` resurrecting a one-sided transfer pair if the pre-existing pairing was unlinked before undo; the aggregate `SyncOutcome.insertedCount` not being adjusted for promoted rows even though the per-account figures were; `recheckContentDedup` able to sacrifice a promotion-candidate row to an unrelated posted duplicate sharing its content signature during the staging-to-write race window; and `linkTransfersByBucket` being called with a pairing floor (`startIso`) that has nothing to do with a promoted row's own date, so a pending row that sat unconfirmed while other, newer activity posted on the same account could be silently, permanently excluded from transfer pairing. All four are fixed with regression tests confirmed to fail without the fix and pass with it. See `docs/plans/sync-pending-promotion.md` for the full round-by-round history.

- [ ] **P4 — accepted residual, NOT fixed: `/sync`'s "Last sync: N transactions" and `/import/success/[batchId]`'s "imported: M" can show two different numbers for the SAME sync batch when it contains a promotion (red-team finding, third convergence pass, `/ship`).** `findLastSyncBatch` (`src/lib/simplefin/undoSync.ts`) deliberately folds `insertedCount + promotedCount` into one `transactionCount` for `/sync`'s own summary — but `syncSimpleFin` persists `import_batches.transactionCount` as INSERT-ONLY (matching CSV's own convention, fixed earlier in this same review pass so `/import/success/[batchId]`'s `autoCategorized`/`left to categorize` math — scoped to batch-owned rows only — doesn't overcount). Both numbers are individually correct for what they measure; the gap is that nothing reconciles them across the two surfaces a user can view back to back (the `/sync` page links directly to `/import/success/[batchId]` for the same batch). Not data corruption, no money-safety impact — purely a "why do these two pages disagree" UX inconsistency. Two fix directions, not chosen between yet: (a) have `/import/success/[batchId]` add its own `sync_promotions` count when rendering "imported", matching `findLastSyncBatch`'s folded semantics, or (b) split `findLastSyncBatch`'s `transactionCount` into separate `insertedCount`/`promotedCount` fields and have `/sync`'s copy show them distinctly (arguably the more honest fix, given this file's own doctrine elsewhere that a promotion is "not new money" and should never be folded into an insert count silently). Deferred rather than fixed under review pressure at the end of an already-large branch (this project's own established pattern for exactly this situation — see e.g. the RememberCheckbox touch-target entries earlier in this file). (`src/lib/simplefin/undoSync.ts`, `src/app/import/success/[batchId]/page.tsx`, `src/app/sync/page.tsx`)

- [ ] **P3 — accepted residual, NOT fixed: undoing a promoted, later-rejected transfer pair leaves the `transfer_pair_rejections` row behind (Codex structured review [P2], `/ship`).** Scenario: a sync promotes row P and pairs it with partner Q; the user later decides this isn't a transfer and clicks "Not a transfer" (`unlinkTransferPair`), which clears both legs' `transfer_pair_id` and records a `transfer_pair_rejections` row for (P, Q); the user then undoes the ORIGINAL sync. `revertPromotion` correctly reverts P to pending (no dangling `transfer_pair_id`, since `priorTransferPairId` and the current value both read null by that point), but the rejection row itself survives — undo reverts a promoted row rather than deleting it, so the `ON DELETE CASCADE` that would have cleaned up the rejection for an ORDINARY (inserted, then deleted-on-undo) row never fires here. If P is promoted again by a later sync and matches Q's content again, `isRejected(P, Q)` still reads true and the pair is refused auto-linking, even though the sync that originally surfaced the pairing has been completely undone. **Deliberately not fixed this cycle**: unlike the four CRITICAL fixes in this same review pass, this is a genuine product judgment call rather than a clear-cut bug — rule 4's own documented design treats a rejection as a durable fact about the TRANSACTIONS ("accumulates forever by design"), not about the sync event that happened to surface the pairing, so deleting it on every promotion-undo could just as easily be the wrong direction. Needs a deliberate decision (does "undo the sync" mean "undo everything that sync's existence made possible," including a rejection recorded afterward?) rather than a reflexive fix under continued review pressure at the end of an already-large branch. (`src/lib/simplefin/undoSync.ts`, `src/lib/transferRejections.ts`)
