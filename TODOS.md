...
- [ ] **P3** — **`/transactions` cannot disable the Remember checkbox the way `/categorize` does, so it warns after the fact.** `loadMerchantGroups` gained a `filedCategoryIds` follow-up query, which is cheap because that page is already grouped by merchant. `loadTransactions` is not: it returns up to `MAX_PAGE_SIZE` individual rows, so the equivalent would be a grouped query over the page's distinct merchants on every render. The server enforces the guard either way — `categorizeTransaction` refuses the rule and returns `ruleRefusal`, which the row renders as a `toast.warning` — so the outcome is correct and explained, just explained one click later than on `/categorize`. Fix by adding the same `inArray` aggregate over the page's merchant set to `loadTransactions` and passing a verdict per row. Blocked by: nothing; it is a cost question, not a design one. (`src/lib/categorize/loadTransactions.ts`, `src/app/transactions/_transaction-row.tsx`)

- [ ] **P3** — **The pre-UPDATE trainability read is deliberately conservative in one case, and it is untested against a user complaint.** `categorizeTransaction` resolves the verdict before its own UPDATE, so a row that is the key's ONLY filed row and is being MOVED from one category to another still counts the category it is leaving: the union is two, the rule is refused, even though after the move the key is unanimous and one rule would be right. Pinned by "MOVING a row off its only category does not refuse on the category it left" in `categorizeTransaction.test.ts` so a future change here is deliberate rather than accidental. Reachable today on `JACK IN THE BOX` (Dining 4 / Fast Food 6) and nowhere else, and the workaround is to re-tick Remember on a second row afterwards. Fixing it properly means computing the post-UPDATE filing set rather than the pre-UPDATE one, which is a different query, not a reordering. (`src/lib/categorize/categorizeTransaction.ts`)

- [ ] **P4** — **`LOSSY_MERCHANT_KEYS` is a curated set and nothing tells you when it is stale.** The three entries have live evidence behind them today (`ONLINE` 39 rows, `MOBILE` 27, and the empty key at 0 rows but reachable per `merchantLabel`'s existence). A future `normalize.ts` change could create a fourth lossy key, or make `ONLINE`/`MOBILE` unreachable, and nothing would say so — the same staleness class CLAUDE.md rule 10 documents for `normalized_merchant` generally. Cheapest detector: have `db:backfill-merchants` report any single-token key above a row-count threshold that carries more than N distinct `raw_memo` shapes, alongside its existing collision report. Not urgent — the cost of a missing entry is one trainable key that should not be, which the multi-category half often catches anyway. (`src/lib/categorize/keyTrainability.ts`, `scripts/backfill-merchants.src.mjs`)
...
a decision (D1=C) and fixed here: the refusal now DELETES a contradicting exact
rule instead of leaving it standing. The rest are below.

- [ ] **P1** — **`/subscriptions` is a third `bulkCategorize` caller with `rememberMerchant: true` that throws the result away, so its refusals are invisible.** `categorizeSubscriptionAction` (`actions.ts:53`) and `categorizeAllSubscriptionsAction` (`:73`) both call `bulkCategorize(db, {..., rememberMerchant: true})` and ignore the return value entirely — no `ruleRefusal`, and now no `refusalDeletedRule` either. `loadSubscriptions` does NOT filter on `categoryId`, so a detected subscription whose rows are already filed elsewhere (NETFLIX under Entertainment, say) hits the multi-category branch, and as of this branch that also removes the rule it had. The page reports plain success either way. "Categorize all" loops over every active subscription doing this, so one click can withhold — and now delete — rules for many merchants with no feedback of any kind. This is the exact silent-rule failure `keyTrainability.ts:5-9` says the guard exists to eliminate, reintroduced at the one call site nobody wired the refusal into. Both actions are `formData` handlers whose return value the page does not currently read, so the fix needs a surface: a per-subscription badge, or a summary line naming the merchants whose rule was withheld. Blocked by: a decision on where "Categorize all" reports N outcomes. (`src/app/subscriptions/actions.ts`, `src/app/subscriptions/page.tsx`)

- [ ] **P2** — **`loadFiledCategoryIds` counts filings under ARCHIVED categories as contradicting evidence.** The WHERE at `resolveKeyTrainability.ts:28-34` filters only on `normalizedMerchant`, `categoryId IS NOT NULL` and `transferPairId IS NULL`; `loadFiledCategories` in `loadMerchantGroups.ts` uses the same predicate. Per rule 8 an archived category's rules never fire (`buildRuleMatcher` skips them), so an archived category cannot compete with a new exact rule and cannot make one wrong. Scenario: archive "Old Subscriptions", which still holds 30 NETFLIX rows, then file new NETFLIX rows to "Streaming" with Remember ticked — `distinct.size === 2`, the checkbox renders disabled and the write is refused, telling the user the key "is already filed under 2 different categories" when only one of them is live. As of this branch it would also delete the NETFLIX rule. Fix: join `categories` and add `isNull(archivedAt)` to BOTH predicates — they must change together or the checkbox and the server verdict diverge, which is the divergence `resolveKeyTrainability.test.ts`'s parity test exists to catch. Not fixed in-branch because it needs the same-predicate-in-two-places problem below decided first. Blocked by: nothing. (`src/lib/categorize/resolveKeyTrainability.ts`, `src/lib/categorize/loadMerchantGroups.ts`)

- [ ] **P2** — **The filed-rows predicate is written out by hand in two places, held together by comments and one test.** `loadFiledCategories` (`loadMerchantGroups.ts:164`) and `loadFiledCategoryIds` (`resolveKeyTrainability.ts:21`) are independent spellings of the same `SELECT DISTINCT category_id` + same three-clause WHERE, differing only in `eq()` vs `inArray()` scope. Both files carry comments warning that a divergence renders an enabled checkbox whose write is then refused, and `resolveKeyTrainability.test.ts:298` pins the agreement — but nothing structural stops an edit to one from missing the other, and the archived-category fix above is exactly such an edit. CLAUDE.md's own precedent for this class is a shared predicate builder (`loadTransactions`/`summarizeByCategory` share `buildPredicates`), not paired comments. Either export the shared WHERE fragment and `and()` it with the per-call scope, or keep only the batched form and define the single-key one as `loadFiledCategories(db, [key]).get(key) ?? []`. ~16 lines removable. Blocked by: nothing. (`src/lib/categorize/loadMerchantGroups.ts`, `src/lib/categorize/resolveKeyTrainability.ts`)

- [ ] **P3** — **The empty key `""` is in `LOSSY_MERCHANT_KEYS` but unreachable on the bulk path, and reachable-but-broken on undo.** `bulkCategorizeInputSchema` pipes `normalizedMerchant` through `z.string().min(1)`, so `bulkCategorizeMerchantAction` throws `Invalid bulk categorize input` before the refusal can fire — the empty-key test in `bulkCategorize.test.ts` calls the function directly and documents a branch the action cannot reach. On the `/transactions` path the key IS reachable (a blank Memo normalizes to `""`), but `categorizeTransactionSnapshotSchema` also declares `normalizedMerchant: z.string().min(1)`, so `undoCategorizeTransactionAction` throws `Invalid undo snapshot` for exactly the row the refusal just handled. Decide whether the empty key is a first-class key everywhere (drop both `min(1)`s) or nowhere (drop it from the lossy set and let the input validators own it), then make the tests match. Blocked by: that one decision. (`src/lib/categorize/validateBulkCategorizeInput.ts`, `src/lib/categorize/validateCategorizeTransactionSnapshot.ts`, `src/lib/categorize/keyTrainability.ts`)

- [ ] **P3** — **`checked={remember && trainability.trainable}` masks the tick rather than clearing it, so it resurrects.** Tick Remember, pick a category that makes the key untrainable (the box visibly un-ticks and disables), then pick a trainable category again — the box re-appears checked with no user action. The `&&` mask leaves `remember` true underneath, so the component shows a control silently re-arming a write the user last saw refused. Either clear the intent on the transition to untrainable, or keep the resurrection and make it legible. Blocked by: nothing. (`src/app/categorize/_merchant-row.tsx:221`)

- [ ] **P3** — **`src/app/categorize/actions.ts` has no test file, while its `/transactions` sibling does.** `bulkCategorizeMerchantAction` now returns `ruleRefusal` and `refusalDeletedRule`, and `_merchant-row.tsx` reads both off it, but nothing pins the JSON round-trip or the snapshot's exclusion of them the way `src/app/transactions/actions.test.ts` does for its half. Mirror that file's shape (validate → bulkCategorize → snapshot → undo, minus the Next.js shell). Blocked by: nothing. (`src/app/categorize/actions.ts`)

- [ ] **P4** — **The verdict→refusal derivation is copy-pasted into both write paths.** `const verdict = rememberMerchant ? resolveKeyTrainability(...) : null; const ruleRefusal = verdict !== null && !verdict.trainable ? verdict : null;` appears identically at `bulkCategorize.ts:161` and `categorizeTransaction.ts:145`, and each site then repeats the `if (rememberMerchant && ruleRefusal === null)` gate. Add `resolveRuleRefusal(db, key, categoryId, rememberMerchant): TrainabilityRefusal | null` beside `resolveKeyTrainability`. ~6 lines removable; the `readExactRule`/`toPriorRuleSnapshot`/`restorePriorRule` extractions in this branch already took the larger half of this duplication. Blocked by: nothing. (`src/lib/categorize/resolveKeyTrainability.ts`)

- [ ] **P4** — **The refusal toast wording lives in two client files.** `_merchant-row.tsx` and `_transaction-row.tsx` both hardcode the same `"Existing rule removed." / "Rule not saved."` branch, the same `toast.warning` channel and the same `duration: 10_000`. The user-facing sentence for a server-side refusal now has to be edited in two places that have no other reason to change together. Extract `notifyRuleRefusal(refusal, deletedRule)`. Blocked by: nothing. (`src/app/categorize/_merchant-row.tsx`, `src/app/transactions/_transaction-row.tsx`)
