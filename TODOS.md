...
- [ ] **P2** — **`/transactions` still cannot disable the Remember checkbox the way `/categorize` does, so it warns after the fact.** (Merged 2026-09-09: filed twice, once at P3 in the round-2 triage above — that copy self-labelled "superseded" but stayed unchecked, so it read as a second open item.) `loadMerchantGroups` gained a `filedCategoryIds` follow-up query, which is cheap because that page is already grouped by merchant. `loadTransactions` is not grouped and returns up to `MAX_PAGE_SIZE` individual rows, so the equivalent is a per-key aggregate over the page's distinct merchants on every render — worth measuring before building. The server enforces the guard either way; what is missing is only the client's ability to disable rather than warn. The fix is an `inArray` aggregate over the page's merchant set in `loadTransactions`, passing a verdict per row. Until then the toast is that surface's only channel, which is why the two toasts had to be merged into one (a non-front toast's action button is drawn at `opacity: 0` by Sonner's collapsed stack). Blocked by: nothing; it is a cost question, not a design one. (`src/lib/categorize/loadTransactions.ts`, `src/app/transactions/_transaction-row.tsx`)
