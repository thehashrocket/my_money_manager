#!/usr/bin/env node
/**
 * Bundles the scripts the Docker runner image needs to run standalone, with no
 * src/ tree and no devDependencies: docker/entrypoint.mjs (the container CMD),
 * scripts/snapshot-cli.mjs (db:export's in-container half), and
 * scripts/backfill-merchants.mjs (db:backfill-merchants' in-container half,
 * which needs src/lib/normalize.ts). All outputs are gitignored build
 * artifacts — see docker/
...
const targets = [
  ...
  {
    entry: "scripts/backfill-merchants.src.mjs",
    outfile: "scripts/backfill-merchants.mjs",
  },
];
